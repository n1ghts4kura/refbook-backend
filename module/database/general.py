...
_user_detail_database = tinydb.TinyDB(USER_DETAIL_DB_PATH, indent=4, ensure_ascii=False)

def get_user_detail_database():
    """获取用户详情数据库实例"""
    return _user_detail_database

_book_database = tinydb.TinyDB(BOOK_DB_PATH, indent=4, ensure_ascii=False)

def get_book_database():
    """获取图书数据库实例"""
    return _book_database

_chat_history_database = tinydb.TinyDB(CHAT_HISTORY_DB_PATH, indent=4, ensure_ascii=False)
